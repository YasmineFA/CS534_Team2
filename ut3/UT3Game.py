...
class UT3Game(Game):
    ...
    def getCanonicalForm(self, board, player):
    	...
    	arr.append(board[1])
    	return np.array(arr) #np.where((board >= 1.0) & (board < numPlayers+1), (board%numPlayers)+1 , board)
...
def display(board, indent='  '):
    		...
    		for xX in range(b_sz):
    			...
    			for yY in range(b_sz):
    				...
    				if (x, y) in windex:
    					...
    					if mask == 1:
    						adder = Back.RED + str(b.pieces[x*b_sz+xX][y*b_sz+yY]).replace('1.0', ' X ').replace('2.0', ' O ').replace('3.0', ' ^ ') + Back.RESET
    					elif mask == 2:
    						adder = Back.BLUE + str(b.pieces[x*b_sz+xX][y*b_sz+yY]).replace('1.0', ' X ').replace('2.0', ' O ').replace('3.0', ' ^ ') + Back.RESET
    					elif mask == 3:
    						adder = Back.GREEN + str(b.pieces[x*b_sz+xX][y*b_sz+yY]).replace('1.0', ' X ').replace('2.0', ' O ').replace('3.0', ' ^ ') + Back.RESET
    					...
    					if temp == 1.0:
    						adder = Back.RED + str(b.pieces[x*b_sz+xX][y*b_sz+yY]).replace('1.0', ' X ') + Back.RESET
    					elif temp == 2.0:
    						adder = Back.BLUE + str(b.pieces[x*b_sz+xX][y*b_sz+yY]).replace('2.0', ' O ') + Back.RESET
    					elif temp == 3.0:
    						adder = Back.GREEN + str(b.pieces[x*b_sz+xX][y*b_sz+yY]).replace('3.0', ' ^ ') + Back.RESET
    					else:
    ...
    for r in range(b_sz**2):
    	...
    	if r%b_sz == b_sz-1:
    		if (r != 0) & (r != (b_sz**2)-1):
    			rows.append('    ' + ('='*((4*b_sz)-1) + '#')*(b_sz-1) + '='*((4*b_sz)-1))
    	else:
    		rows.append(rowSep)
